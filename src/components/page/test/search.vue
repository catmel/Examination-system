...
export default {
    ...
    methods: {
        querySearch(queryString, cb) {
            ...
            // this.$store.dispatch('users/search_user', {keyword: queryString}).then({})

            
console.log(this.state);
            var restaurants = this.restaurants;
            console.log("所有数据",restaurants);

            // console.log("匹配数据",restaurants.filter(this.createFilter(queryString)));
            // console.log(">>>",this.createFilter(queryString));
            var results = queryString
                ? restaurants.filter(this.createFilter(queryString))
                : restaurants;
            console.log("显示的匹配内容",results);
            cb(results);
        },
        createFilter(queryString) {
            return restaurant => {
                //判断是否有搜索内容---true/false
                return (
                    restaurant.value
                        .toLowerCase()
